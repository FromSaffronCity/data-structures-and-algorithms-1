...
/* global variable for counting inversions */
...
void merge(int* array, int start, int mid, int end) {
    ...
    /* Counting inversions by sorting in ascending order of integer values */
    while(index1<amount1 && index2<amount2) {
        if(tempArray1[index1] > tempArray2[index2]) {
            array[indexMerged++] = tempArray2[index2++];

            inversion += (amount1 - index1);  /* NOTICE - main activity here */
        }
        else {
